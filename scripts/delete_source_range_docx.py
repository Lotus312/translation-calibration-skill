...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Delete source-language-only DOCX paragraphs in a confirmed range."
    )
    ...
    if args.section_text:
        ...
        if args.next_section_text:
            end_visible_idx = find_index(visible_texts, args.next_section_text, start_visible_idx + 1) - 1
        else:
